...
import { Bell, BellOff, X, Check, CheckCheck } from 'lucide-react';
...
// Module-level counter: survives component unmount/remount so channel names
// are always unique even when React StrictMode mounts the component twice.
let _bellChannelSeq = 0;

const PREVIEW_ITEMS = [
  { id: 'pr1', is_read: false, notifications: { id: 'n1', title: 'Chào mừng đến với Projexa!', body: 'Hệ thống quản lý dự án nội bộ đã sẵn sàng phục vụ bạn.', created_at: new Date().toISOString() } },
  { id: 'pr2', is_read: false, notifications: { id: 'n2', title: 'Sprint 3 đã được khởi động', body: 'Sprint 3 của dự án CPLQG vừa được tạo và bắt đầu từ hôm nay.', created_at: new Date(Date.now() - 3600000).toISOString() } },
  { id: 'pr3', is_read: true,  notifications: { id: 'n3', title: 'Cập nhật tính năng hệ thống', body: 'Phiên bản mới đã được triển khai với nhiều cải tiến về giao diện.', created_at: new Date(Date.now() - 86400000).toISOString() } },
];
...
export default function NotificationBell() {
  ...
  // Channel name includes a counter so each effect invocation gets a fresh channel,
  // avoiding the "cannot add callbacks after subscribe()" error in React StrictMode.
  useEffect(() => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-rose-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-1 leading-none shadow-sm">
            {unreadCount > 9 ? '9+' : unreadCount}
          </span>
        )}
      </button>

      {/* Dropdown panel — opens upward to the left */}
      {isOpen && (
          ...
          <div className="max-h-[320px] overflow-y-auto">
            {loading ? (
              ...
              items.map(rec => (
                <div
                  key={rec.id}
                  className={`flex gap-3 px-4 py-3 border-b border-slate-100 dark:border-gray-800/60 last:border-0 transition-colors ${
                    !rec.is_read
                ...
                >
                  ...
                  {!rec.is_read && (
                    <button
                      ...
                      className="shrink-0 self-center p-1.5 text-slate-300 dark:text-gray-600 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors"
